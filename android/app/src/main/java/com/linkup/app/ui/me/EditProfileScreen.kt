...
fun EditProfileScreen(
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(20.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
            TextButton(onClick = onBack, enabled = !busy) { Text("‹", color = LinkUpRed, fontSize = 24.sp) }
            Text("Edit profile", color = LinkUpTextPrimary, fontWeight = FontWeight.Bold, modifier = Modifier.weight(1f))
            TextButton(
                onClick = { onSave(name.trim(), avatar.trim(), visibility, language) },
                enabled = !busy && nameLength in 1..80 && avatar.trim().length <= 2048,
            ) { Text(if (busy) "Saving…" else "Save", color = LinkUpRed) }
        }
        ...
        OutlinedTextField(value = avatar, onValueChange = { avatar = it }, enabled = !busy,
            label = { Text("Avatar URL (optional)") }, singleLine = true, modifier = Modifier.fillMaxWidth())
        Text("Leave the avatar URL empty to remove it.", color = LinkUpTextMuted, fontSize = 12.sp)
        Text("Profile visibility", color = LinkUpTextPrimary, fontWeight = FontWeight.Bold)
        listOf("PUBLIC" to "Public", "HIDDEN" to "Hidden").forEach { (value, label) ->
            Row(verticalAlignment = Alignment.CenterVertically) {
                RadioButton(selected = visibility == value, enabled = !busy, onClick = { visibility = value })
                Text(label, color = LinkUpTextPrimary)
            }
        }
        Text("Preferred language", color = LinkUpTextPrimary, fontWeight = FontWeight.Bold)
        listOf("uk" to "Українська", "en" to "English").forEach { (value, label) ->
            Row(verticalAlignment = Alignment.CenterVertically) {
