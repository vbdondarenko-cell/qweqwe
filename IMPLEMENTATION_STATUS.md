...
No new migration is required for this block (only SQL string constants and Go query logic change; the runtime table these queries read, `slot_selected_viewers`, was already created by migration `000032` in §59). Production deployment for this block follows immediately in this same session; if this paragraph has not yet been replaced by a live-verified deployment addendum below, treat that half of the work as not yet done — not assumed.
