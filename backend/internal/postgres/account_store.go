...
func (s *AccountStore) CreateSession(ctx context.Context, sess account.Session, expectedPasswordHash string) error {
	tx, err := s.pool.BeginTx(ctx, pgx.TxOptions{IsoLevel: pgx.ReadCommitted})
	if err != nil {
		return err
	}
	...
	err = tx.QueryRow(ctx, `SELECT password_hash FROM app_users WHERE id=$1 FOR UPDATE`, sess.UserID).Scan(&currentPasswordHash)
	if errors.Is(err, pgx.ErrNoRows) {
	...
	if _, err := tx.Exec(ctx, `INSERT INTO user_sessions (id,user_id,token_hash,created_at,expires_at,device_label) VALUES ($1,$2,$3,$4,$5,NULLIF($6,''))`, sess.ID, sess.UserID, sess.TokenHash, sess.CreatedAt, sess.ExpiresAt, sess.DeviceLabel); err != nil {
		return mapError(err)
	}
	return tx.Commit(ctx)
}
...
func (s *AccountStore) UpdateProfile(ctx context.Context, userID string, p account.ProfilePatch, now time.Time) (account.User, error) {
	...
	displaySet := p.DisplayName != nil
	display := ""
	if p.DisplayName != nil {
		display = *p.DisplayName
	}
	avatarSet := p.AvatarURL != nil
	avatar := ""
	if p.AvatarURL != nil {
		avatar = *p.AvatarURL
	}
	visibilitySet := p.ProfileVisibility != nil
	visibility := ""
	if p.ProfileVisibility != nil {
		visibility = *p.ProfileVisibility
	}
	languageSet := p.Language != nil
	language := ""
	if p.Language != nil {
		language = *p.Language
	}
...
func (s *AccountStore) CreatePasswordReset(ctx context.Context, reset account.PasswordReset, now time.Time) error {
	...
	err = tx.QueryRow(ctx, `SELECT id FROM app_users WHERE id=$1 FOR UPDATE`, reset.UserID).Scan(&lockedUserID)
	if errors.Is(err, pgx.ErrNoRows) {
		return account.ErrNotFound
	}
	if err != nil {
	...
	if _, err = tx.Exec(ctx, `UPDATE password_reset_tokens SET used_at=$2 WHERE user_id=$1 AND used_at IS NULL`, reset.UserID, now); err != nil {
		return err
	}
	if _, err = tx.Exec(ctx, `INSERT INTO password_reset_tokens (id,user_id,token_hash,created_at,expires_at) VALUES ($1,$2,$3,$4,$5)`, reset.ID, reset.UserID, reset.TokenHash, reset.CreatedAt, reset.ExpiresAt); err != nil {
		return mapError(err)
	}
	return tx.Commit(ctx)
}
...
func (s *AccountStore) ResetPassword(ctx context.Context, tokenHash []byte, newPasswordHash string, now time.Time) error {
	...
	var userID string
	err = tx.QueryRow(ctx, `SELECT user_id FROM password_reset_tokens WHERE token_hash=$1 AND used_at IS NULL AND expires_at>$2`, tokenHash, now).Scan(&userID)
	if errors.Is(err, pgx.ErrNoRows) {
		return account.ErrUnauthorized
	}
	if err != nil {
	...
	var lockedUserID string
	err = tx.QueryRow(ctx, `SELECT id FROM app_users WHERE id=$1 FOR UPDATE`, userID).Scan(&lockedUserID)
	if errors.Is(err, pgx.ErrNoRows) {
		return account.ErrUnauthorized
	}
	if err != nil {
	...
	err = tx.QueryRow(ctx, `UPDATE password_reset_tokens SET used_at=$2 WHERE token_hash=$1 AND used_at IS NULL AND expires_at>$2 RETURNING user_id`, tokenHash, now).Scan(&confirmedUserID)
	if errors.Is(err, pgx.ErrNoRows) {
		return account.ErrUnauthorized
	}
	if err != nil {
	...
	if _, err = tx.Exec(ctx, `UPDATE app_users SET password_hash=$2,updated_at=$3 WHERE id=$1`, userID, newPasswordHash, now); err != nil {
		return err
	}
