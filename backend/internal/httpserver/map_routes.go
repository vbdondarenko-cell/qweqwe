...
// mapHistoricalViewport implements README §6.23's LinkUp+ "Pulse
// Time-Machine": the same aggregate cluster query as mapViewport, but
// gated on an active LinkUp+ subscription and accepting a much wider time
// window (up to citymap.HistoricalWindowMax) at a correspondingly coarser
// zoom ceiling once that window is wide — see citymap.Viewport.ValidHistorical.
func (s *Server) mapHistoricalViewport(w http.ResponseWriter, r *http.Request) {
	auth, ok := authFrom(r)
	if !ok {
		writeProblem(w, r, http.StatusUnauthorized, "unauthorized", "authentication required")
		return
	}
	if s.deps.Map == nil || s.deps.CityContext == nil || s.deps.Monetization == nil {
		writeProblem(w, r, http.StatusServiceUnavailable, "not_ready", "map dependencies are unavailable")
		return
	}
	city, err := s.deps.CityContext.Current(r.Context(), auth.User.ID)
	...
	items, err := s.deps.Map.HistoricalViewport(r.Context(), auth.User.ID, city.Locality.ID, citymap.Viewport{
		WestE6: west, SouthE6: south, EastE6: east, NorthE6: north,
		Zoom: zoom, From: from, To: to, Limit: limit,
	}, snapshot.Status.PremiumActive)
	if err != nil {
		switch err {
		case citymap.ErrPremiumRequired:
			writeProblem(w, r, http.StatusForbidden, "premium_required", "Pulse Time-Machine requires an active LinkUp+ subscription")
		case citymap.ErrInvalidViewport:
			writeProblem(w, r, http.StatusBadRequest, "invalid_viewport", "invalid map viewport")
		default:
			writeProblem(w, r, http.StatusInternalServerError, "internal_error", "request failed")
		}
